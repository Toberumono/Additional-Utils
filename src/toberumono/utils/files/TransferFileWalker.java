...
public class TransferFileWalker extends LoggedFileWalker {
	...
	public static interface TransferAction extends IOExceptedBiFunction<Path, Path, Path> {
		/**
		 * Transfers the file at {@code source} to {@code target}
		 * 
		 * @param source
	...
	/**
	 * Construct a new {@link TransferFileWalker} with the given {@code target} and {@code action}.<br>
	 ...
	 * @see BasicTransferActions
	 */
	public TransferFileWalker(Path target, TransferAction action, Level normalLoggingLevel, Level issueLoggingLevel) {
		this(target, action, null, null, null, normalLoggingLevel, issueLoggingLevel);
	}
